...
<use xlink:href="#K" transform="rotate(-135,32,32)"/>`;
    for (let i = 1; i < fr; ++i)
      r += `<use xlink:href="#K" transform="translate(0,${
        64 * i
      }) rotate(${-135 + (270 * i) / fr},32,32)"/>`;
    return r + '</svg>';
  };
...
<defs><rect id="B" x="0" y="0" width="${w}" height="${h}" rx="${
      w / 2
    }" ry="${w / 2}" fill="${bg}"/>
<circle id="K" x="0" y="0" r="${
      (w / 2) * 0.9
    }" fill="${fg}"/></defs>`;
    for (let i = 0; i < fr; ++i) {
      r += `<use xlink:href="#B" transform="translate(0,${h * i})"/>`;
  ...
  let initSwitches = (el) => {
    ...
    el.refresh = () => {
      ...
      h = parseFloat(
        el.getAttribute('data-height') || d || st.height
      );
...
<g><rect fill="${bg}" x="1" y="1" width="${w - 2}" height="${
          h - 2
        }" rx="${minwh * 0.25}" ry="${minwh * 0.25}"/>
<rect fill="${bg}" x="1" y="${h + 1}" width="${w - 2}" height="${
          h - 2
        }" rx="${minwh * 0.25}" ry="${minwh * 0.25}"/>
<circle fill="${fg}" cx="${w * 0.5}" cy="${h * 1.5}" r="${
  ...
  let initKnobs = (el) => {
    ...
    ik.wheel = (ev) => {
      let delta =
        ev.deltaY > 0 ? -ik.valrange.step : ik.valrange.step;
